describe('KeyPressHandler', function() {
    ...
    beforeEach(inject(function($injector) {
        keyPressHandler = $injector.get("KeyPressHandler")
    }));
    ...
    describe('getNextMovement', function() {
        ...
        it('should return direction and then no direction if movement key quickly pressed and released before called getNextMovement', function() {
            ...
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Left, isFiring: false} );
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.None, isFiring: false} );
        });
        ...
        it ('should continue to return movement until the key is released', function() {
            ...
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Down, isFiring: false} );
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Down, isFiring: false} );
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Down, isFiring: false} );
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Down, isFiring: false} );
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Down, isFiring: false} );
        ...
        it ('should continue to return movement all the time the key is pressed', function() {
            ...
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Right, isFiring: false} );
            keyPressHandler.keyPress(39);
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Right, isFiring: false} );
            keyPressHandler.keyPress(39);
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Right, isFiring: false} );
        ...
        it ('should return firing information as well as movement information', function() {
            ...
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Up, isFiring: false} );
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Up, isFiring: false} );

            // moving up and firing
            keyPressHandler.keyPress(32);
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Up, isFiring: true} );
            expect(keyPressHandler.getNextMovement()).toEqual( { direction: DirectionEnum.Up, isFiring: true} );
